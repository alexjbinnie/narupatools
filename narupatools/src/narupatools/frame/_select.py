...
from narupatools.frame.fields import ParticleResidues, ResidueNames, ParticleNames, ResidueIds, ParticleTypes, ParticlePositions, BondPairs
...
def select(frame: Any, /, selection: str):
    ...
    fields = set()
    if {'protein', 'backbone', 'nucleic', 'nucleicbackbone', 'nucleicsugar', 'resname', 'atom'} & keywords:
    ...
    if {'around', 'sphzone', 'sphlayer', 'cyzone', 'cylayer', 'point', 'prop'} & keywords:
    ...
    if missing_keys := fields - set(frame.keys()):
        raise ValueError(f"Cannot use selection '{selection}' - key(s) {missing_keys} are unavailable.")
