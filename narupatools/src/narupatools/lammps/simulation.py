...
class LAMMPSSimulation:
    ...
    def extract_compute(
        ...
        key: str,
        style: VariableStyle,
        dimension: Literal[VariableDimension.VECTOR1D],
    ) -> np.ndarray:
        ...
    ...
    def extract_compute(
        ...
        key: str,
        style: VariableStyle,
        dimension: Literal[VariableDimension.ARRAY2D],
    ) -> np.ndarray:
        ...

    def extract_compute(
        self, key: str, style: VariableStyle, dimension: VariableDimension
        ...
        """
        value = self.__lammps.lmp.numpy.extract_compute(key, style, dimension)
        if value is None:
            computes = self.__lammps.computes
            if not any(compute["name"] == key for compute in computes):
                raise ComputeNotFoundError(key)
            else:
                raise InvalidComputeSpecificationError(key, style, dimension)
        return value

    def gather_atoms(
        self, key: str, datatype: VariableType, dimension: int
    ) -> np.ndarray:
        """
        ...
        """
        dtype = float if datatype == VariableType.DOUBLE else int
        try:
            with catch_lammps_warnings_and_exceptions():
                natoms = self.__lammps.lmp.get_natoms()
                data = ((dimension * natoms) * datatype.get_ctype())()

                self.__lammps.lmp.lib.lammps_gather_atoms(
                    self.__lammps.lmp.lmp, key.encode(), datatype, dimension, data
                )
        ...
        if self._potential_energy is None:
            self._potential_energy = (
                self.extract_compute(
                    "thermo_pe", VariableStyle.GLOBAL, VariableDimension.SCALAR
                )
    ...
    def kinetic_energy(self) -> float:
        ...
        if self._kinetic_energy is None:
            self._kinetic_energy = (
                self.extract_compute(
                    "thermo_ke", VariableStyle.GLOBAL, VariableDimension.SCALAR
                )
                * self._lammps_to_narupa.energy
            )
    ...
    def temperature(self) -> float:
        ...
        if self._temperature is None:
            self._temperature = (
                self.extract_compute(
                    "thermo_temp", VariableStyle.GLOBAL, VariableDimension.SCALAR
                )
    ...
    def pressure(self) -> float:
        ...
        if self._pressure is None:
            self._pressure = (
                self.extract_compute(
                    "thermo_press", VariableStyle.GLOBAL, VariableDimension.SCALAR
                )
    ...
    def forces(self) -> np.ndarray:
        ...
        if self._forces is None:
            self._forces = (
                self.gather_atoms("f", VariableType.DOUBLE, 3)
    ...
    def atom_ids(self) -> np.ndarray:
        ...
        if self._atom_ids is None:
            self._atom_ids = self.gather_atoms("id", VariableType.INTEGER, 1)
    ...
    def charges(self) -> np.ndarray:
        ...
        if self._charges is None:
            try:
                self._charges = (
                    self.gather_atoms("q", VariableType.DOUBLE, 1)
    ...
    def masses(self) -> np.ndarray:
        ...
        if self._masses is None:
            self._masses = (
                self.extract_compute(
                    "mass", VariableStyle.ATOM, VariableDimension.VECTOR1D
                )
    ...
    def positions(self) -> np.ndarray:
        ...
        if self._positions is None:
            self._positions = (
                self.gather_atoms("x", VariableType.DOUBLE, 3)
    ...
    def positions(self, positions: np.ndarray) -> None:
        self._scatter_atoms(
            "x", VariableType.DOUBLE, 3, positions * self._narupa_to_lammps.length
        )
    ...
    def velocities(self) -> np.ndarray:
        ...
        if self._velocities is None:
            self._velocities = (
                self.gather_atoms("v", VariableType.DOUBLE, 3)
    ...
    def velocities(self, velocities: np.ndarray) -> None:
        self._scatter_atoms(
            "v", VariableType.DOUBLE, 3, velocities * self._narupa_to_lammps.velocity
        )
    ...
    def get_imd_forces(self) -> np.ndarray:
        ...
        imd_extracted = self.__lammps.lmp.numpy.extract_compute(
            "imdf", VariableStyle.ATOM, VariableDimension.ARRAY2D
        )
    ...
    def _generate_index_to_id(self) -> None:
        ...
        for index, atom_id in enumerate(ids):
            self._index_to_id[index] = atom_id
    ...
    def set_imd_force(self, index: int, force: Vector3) -> None:
        ...
        atom_id = self._index_to_id[index]
        force = force * self._narupa_to_lammps.force
        self.command(
            f"set atom {atom_id} d_imdfx {force[0]} d_imdfy {force[1]} d_imdfz {force[2]}"
        )
        self._clear_cache()
    ...
    def clear_imd_force(self, index: int) -> None:
        ...
        if len(self._index_to_id) == 0:
            self._generate_index_to_id()
        atom_id = self._index_to_id[index]
        self.command(f"set atom {atom_id} d_imdfx 0 d_imdfy 0 d_imdfz 0")
        self._clear_cache()
    ...
    def create_atom(self, *, atom_type: int, position: Vector3) -> None:
        """
    ...
    def set_mass(self, *, atom_type: Union[int, slice], mass: float) -> None:
        """
        ...
        """
        self.command(f"mass {atom_type} {mass * self._narupa_to_lammps.mass}")
